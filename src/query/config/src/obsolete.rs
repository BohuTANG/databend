...
use crate::inner::QueryConfig;

struct ObsoleteItem<T> {
    target: Option<T>,
    key: String,
    cli_arg_name: String,
    alternative_env_name: String,
    alternative_cli_arg_name: String,
    alternative_config_sample: String,
}

impl QueryConfig {
    fn obsoletes<T>(&self) -> Result<Vec<ObsoleteItem<T>>> {
        let result = vec![ObsoleteItem {
            target: self.table_disk_cache_mb_size,
            key: "table_disk_cache_mb_size".to_string(),
            cli_arg_name: "".to_string(),
            alternative_env_name: "".to_string(),
            alternative_cli_arg_name: "".to_string(),
            alternative_config_sample: "".to_string(),
        }];
        todo!()
    }
